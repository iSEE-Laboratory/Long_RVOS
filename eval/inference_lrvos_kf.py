...
import torch
import sys
sys.path.append('/data1/tianming/long_rvos/')
...
def main(args):
    ...
    output_dir = os.path.join(save_dir, "key_frames")
    ...
    for video in video_list:
        ...
        for i in range(num_expressions):
            ...
            if args.visualize:
                mask = pred_masks
                box = pred_boxes
                for t in range(num_key_frames):
                    frame = f"{key_frame_ids[t]:05d}"
                    # original
                    img_path = os.path.join(img_folder, video_name, frame + '.jpg')
            ...
            for t, mask in zip(key_frame_ids, masks):
                frame = frames[t]
