...
def crop(clip, target, region):
    ...
    if "motions" in target:
        target['motions'] = F.crop(target['motions'], *region)
        fields.append("motions")
...
def hflip(clip, target):
    ...
    if "motions" in target:
        target['motions'] = F.hflip(target['motions'])
...
def vflip(clip,target):
    ...
    if "motions" in target:
        target['motions'] = F.vflip(target['motions'])
...
def resize(clip, target, size, max_size=None, binarize=True):
    ...
    if "motions" in target:
        target['motions'] = F.resize(target['motions'], size, interpolation=InterpolationMode.BICUBIC)
...
def pad(clip, target, padding):
    ...
    if "motions" in target:
        target['motions'] = torch.nn.functional.pad(target['motions'], (0, padding[0], 0, padding[1]))
